...
func (ct *ClaimTrie) AppendBlock() ([][]byte, error) {
	...
	if len(ct.changes) > 0 && ct.chainRepo != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("chain change repo save: %w", err)
		}
...
func (ct *ClaimTrie) ResetHeight(height int32) error {
	...
	if err != nil {
		return fmt.Errorf("decrement node manager height to %d: %w", height, err)
	}

	ct.height = height
	hash, err := ct.blockRepo.Get(height)
	if err != nil {
		return fmt.Errorf("get block repo at height %d: %w", height, err)
	}
